...
describe("/api/sync/run", () => {
  ...
  test("rejects a sync rule that does not belong to the user", async () => {
    mocks.syncRuleFindFirst.mockResolvedValue(null);

    const response = await POST(request({ syncRuleId: "other-rule" }));
    const payload = await response.json();

    expect(response.status).toBe(404);
    expect(payload).toEqual({ error: "Sync rule not found" });
    expect(mocks.startBrowserActionJob).not.toHaveBeenCalled();
  });

  test("rejects missing syncRuleId", async () => {
    const response = await POST(request({}));
    const payload = await response.json();

    expect(response.status).toBe(400);
    expect(payload).toEqual({ error: "syncRuleId is required" });
