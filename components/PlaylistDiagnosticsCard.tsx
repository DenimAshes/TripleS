    ...
    <div className="relative mb-6 overflow-hidden rounded-2xl border border-blue-500/10 bg-[#0d0e12]/60 p-6 backdrop-blur-xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="relative z-10">
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-400/80">Playlist cache</div>
          <div className="mt-2 text-sm font-semibold text-white">{status}</div>
        ...
        <div className="relative z-10 text-right">
          <div className={`text-3xl font-black tabular-nums tracking-tighter ${partial ? "text-amber-400" : "text-blue-400"} drop-shadow-[0_0_10px_rgba(59,130,246,0.3)]`}>
            {activeStates}
            <span className="ml-1 text-lg font-bold text-slate-600">/{expected || "-"}</span>
          </div>
        </div>
      </div>
      {expected > 0 ? (
        <div className="relative z-10 mt-5 h-1.5 w-full overflow-hidden rounded-full border border-white/5 bg-black/40">
          <div
