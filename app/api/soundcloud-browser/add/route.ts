...
export async function POST(request: Request) {
  ...
  try {
    const result = await addSoundCloudTrackToPlaylistCli(String(body.playlistId), String(body.trackId));
    if (result.added) await invalidateSoundCloudPlaylistTracks(String(body.playlistId));
